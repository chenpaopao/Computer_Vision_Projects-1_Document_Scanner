...
class Scanner:
	...
	def Scan_View(self, save_collage=False, resize_collage=True, resize_height=500):
		...
		if save_collage:
			...
			cv2.imwrite('./mid/'+'_cov'+self.filename, horiz_conc)
		else:
			# Saving an image itself
			cv2.imwrite('./mid/'+'_cov'+self.filename, image)
		return image
	...
	def Rotation(self, save_rotated=False, resize_height=500):
		...
		if save_rotated:
			# Saving an image itself
			cv2.imwrite('./result/'+self.filename, image)
		return image
...
if __name__=="__main__":
	...
	for filename in os.listdir(r"./" + "source"):
			...
			# help(Scanner)
			scanned_im = './mid/'+'_cov'+filename
			scan = Scanner(scanned_im,filename)
